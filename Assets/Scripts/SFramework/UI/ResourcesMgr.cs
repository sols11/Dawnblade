...
namespace SFramework
{
    ...
    /// 智能清理：若缓冲池的GO还没有active=false时创建新GameObject，并在一定时间后销毁（GameObject自身决定销毁）
    /// GO也可以选择不使用这里提供的缓冲池
    /// </summary>
    public class ResourcesMgr : IGameMgr
    {
        private Hashtable ht = null;                        //读取资源的缓冲池
        ...
        /// <param name="isCatch">是否加入缓冲池</param>
        /// <returns></returns>
        public GameObject LoadAsset(string path, bool isCatch,Vector3 _position,Quaternion _rotation,int autoDestroyTime=10)
        {
            // 字典中已有的物体
            if (dicGO.ContainsKey(path))
            {
                ...
                // 否则就创建一个新的go，并在一定时间后销毁
                GameObject _goObj = GameObject.Instantiate(dicGO[path], _position, _rotation);
                // autoDestroyTime后清理掉多出来的GO，<=0表示不销毁
                if (autoDestroyTime>0)
                    GameObject.Destroy(_goObj, autoDestroyTime); 
                return _goObj;
            }
            // 没有的话就Resources.Load一个新的并加入字典
            GameObject goObj = Resources.Load<GameObject>(path);
            if (goObj != null)
            {
