...
namespace SFramework
{
	...
	public class PlayerMgr : IGameMgr
	{
	    ...
	    public void BuildPlayerPeace(Vector3 position, Quaternion quaternion)
	    {
	        if (CurrentPlayer == null)
	            SetCurrentPlayer(new DreamKeeper.PlayerPeace(GameMainProgram.Instance.resourcesMgr.
	                LoadAsset(@"Players\BlockPeace", false, position, quaternion)));
	        gameMain.gameDataMgr.Load(CurrentPlayer);   // 读档
	    }
